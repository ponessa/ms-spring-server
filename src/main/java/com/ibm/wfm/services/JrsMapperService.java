...
public class JrsMapperService {
	...
	public static void main(String[] args) {
		...
		// String mapFileName =
		// "/Users/steve/$WFM/infinity/practice_map/JRS_MAP_v37.csv";
		// String saMapFileName =
		// "/Users/steve/$WFM/infinity/practice_map/SERVICE_AREA_MAP_v37.csv";
		...
		if (!validParams) {
			...
			System.out.println("Project: ms-spring-server (Microservice Spring Server)");
			System.out.println("");
	...
	public ArrayList<JrsMapNode> buildJrsMapFromFile(String mapFileName, String delimiter) {
		...
		for (String[] columns : rows) {
			if (++rowCnt > 1) {
				...
				JrsMapNode jrsMap = new JrsMapNode(columns[0], columns[1], columns[2], columns[3], columns[4],
						columns[5], columns[10], columns[8], columns[7], columns[9], columns[6],
						columns.length > 13 ? columns[13] : "", columns.length > 11 ? columns[11] : "",
						columns.length > 12 ? columns[12] : "", columns.length > 14 ? columns[14] : "");
				if (jrsMaps == null)
					jrsMaps = new ArrayList<JrsMapNode>(rows.size());
				if (!jrsMap.getNewGrowthPlatformNm().trim().equalsIgnoreCase("#N/A")
						&& !jrsMap.getNewGrowthPlatformNm().trim().equalsIgnoreCase("Business Support"))
					jrsMaps.add(jrsMap);
	...
	public ArrayList<ServiceAreaMapNode> buildServiceAreaMapFromFile(String mapFileName, String delimiter) {
		...
		for (String[] columns : rows) {
			if (++rowCnt > 1) {
				...
				ServiceAreaMapNode serviceAreaMapNode = new ServiceAreaMapNode(columns[0], columns[1], columns[2],
						columns[3], columns[4], columns[5], columns[6], columns.length > 7 ? columns[7] : "",
						columns.length > 8 ? columns[8] : "", columns.length > 9 ? columns[9] : "",
						columns.length > 10 ? columns[10] : "", columns.length > 11 ? columns[11] : "");
				if (serviceAreaMap == null)
				...
				if (!serviceAreaMapNode.getNewGrowthPlatformNm().trim().equalsIgnoreCase("#N/A")
						&& !serviceAreaMapNode.getNewGrowthPlatformNm().trim().equalsIgnoreCase("Business Support"))
	...
	private JrsMapperResponse map(String dataFileName, String delimiter, ArrayList<JrsMapNode> jrsMap,
		...
		try {
			...
			System.out.println("                                   " + Helpers.pad(
					Helpers.formatInt(
							unresUnmatched + unresJrsDel + unresJrsMig + unresJrsOos + unresUnknown + unresError),
					8, Helpers.PAD_LEFT));
			System.out.println("");
					...
					.println(
							"Total                              " + Helpers.pad(
									Helpers.formatInt(resolvedJrsChg + resolvedJrsNoChg + resolvedJrsOrgChg
											...
											+ unresJrsDel + unresJrsMig + unresJrsOos + unresUnknown + unresError),
									8, Helpers.PAD_LEFT));
	...
	public String toStringDetailServiceAreaMatch(int resolutionInd, String[] columns, int[] returnOffsets,
		...
		// Add the JRS Map Columns
		sb
				// .append(Helpers.formatCsvField(serviceAreaMapNode.getGrowthPlatformNm())).append(",")
				// .append(Helpers.formatCsvField(serviceAreaMapNode.getServiceLineNm())).append(",")
				// .append(Helpers.formatCsvField(serviceAreaMapNode.getPracticeNm())).append(",")
				// .append(Helpers.formatCsvField(serviceAreaMapNode.getServiceAreaNm())).append(",")
				// .append(" ,") //capacity group
				// .append(" ,") //JrsNm
	...
	public String toStringDetailJrsMatch(String[] columns, int[] returnOffsets, JrsMapNode jrsMapNode, int status,
		...
		if (jrsMapNode.getNewJrsNm() == null || jrsMapNode.getNewJrsNm().trim().length() == 0
			...
			} else if (jrsMapNode.getJrsActionNm().trim().toLowerCase().contains("change")) {
				resolvedInd = 3;
				useJrsNm = jrsMapNode.getJrsNm();
				;
	...
	public int getBurstId(String[] columns) {
		...
		else if (columns[ID_COUNTRY].equals("Austria") && columns[ID_GEOGRAPHY].equals("GIC and CPSD"))  burstId = 2;
		...
		else if (emeaMarketList.contains(columns[ID_MARKET]) && columns[ID_GEOGRAPHY_TYPE].equals("CPS Delivery"))                     burstId = 6;
		else if (emeaMarketList.contains(columns[ID_MARKET]) && columns[ID_GEOGRAPHY_TYPE].equals("Sales & Support") 
				                                             && columns[ID_GEOGRAPHY].equals("CoC"))                                   burstId = 7;
		else if (emeaMarketList.contains(columns[ID_MARKET]) && columns[ID_GEOGRAPHY_TYPE].equals("Sales & Support")
				                                             && columns[ID_GEOGRAPHY].equals("SIH"))                                   burstId = 8;
		else if (emeaMarketList.contains(columns[ID_MARKET]) && columns[ID_GEOGRAPHY_TYPE].equals("Sales & Support")
				                                             && columns[ID_GEOGRAPHY].equals("WW Top"))                                burstId = 9;
		
		/* Brazil */
		else if (columns[ID_COUNTRY].equals("Brazil") && columns[ID_GEOGRAPHY_TYPE].equals("Domestic"))                                burstId = 10;
		else if (columns[ID_COUNTRY].equals("Brazil") && columns[ID_GEOGRAPHY_TYPE].equals("Globally Integrated Capabilities"))        burstId = 11;
		else if (columns[ID_COUNTRY].equals("Brazil") && columns[ID_GEOGRAPHY_TYPE].equals("CPS Delivery"))                            burstId = 12;
		else if (columns[ID_COUNTRY].equals("Brazil") && columns[ID_GEOGRAPHY_TYPE].equals("Sales & Support")
				                                      && columns[ID_GEOGRAPHY].equals("CoC"))                                          burstId = 13;
		/* Americas */
		else if (columns[ID_GEOGRAPHY].equals("Americas")) burstId = 14;
		/* Japan */
		else if (columns[ID_GEOGRAPHY].equals("Japan"))    burstId = 15;
		/* APAC */
		else if (columns[ID_GEOGRAPHY].equals("APAC"))     burstId = 16;
		/* GIC */
			...
			if ((columns[ID_MARKET].equals("Canada") || columns[ID_MARKET].equals("LA") || columns[ID_MARKET].equals("United States"))
				...
				burstId = 17;
		} else if (columns[ID_GEOGRAPHY_TYPE].equals("CPS Delivery"))
			burstId = 18;
